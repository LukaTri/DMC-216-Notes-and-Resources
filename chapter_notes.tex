...
\begin{enumerate}
	\item \emph{State the null (\emph{$H_{0}$}) and research (alternative/\emph{$H_{a}$}) hypothesis.}
		\begin{itemize}
			...
			\item $H_{a}$ indiciates there exists a difference between conditions, groups, or variables.
				...
				\end{itemize}
		\end{itemize}
	\item \emph{Set the level of signifigance (usually, it is 5\%}).\\[2mm]
		\begin{table}[h]
		\centering
		\begin{tabular}{c|c|c}
			...
			\hline
			$H_{0}$ False & Type \Romannum{2} error; $\beta$ & No error\\[2mm]
		\end{tabular}
